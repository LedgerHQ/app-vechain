...
static void processToField(clauseContext_t *context) {
    ...
    if (context->currentFieldPos < context->currentFieldLength) {
        ...
        copyClauseData(context, context->content->to + context->currentFieldPos, copySize);
    }
    if (context->currentFieldPos == context->currentFieldLength) {
    ...
    if (context->currentFieldPos < context->currentFieldLength) {
        uint32_t copySize =
            (context->commandLength < ((context->currentFieldLength - context->currentFieldPos))
                 ? context->commandLength
                 : context->currentFieldLength - context->currentFieldPos);
        copyClauseData(context, context->content->value.value + context->currentFieldPos, copySize);
    }
    if (context->currentFieldPos == context->currentFieldLength) {
        ...
        if (context->currentFieldPos < context->currentFieldLength) {
            uint32_t copySize =
                (context->commandLength < ((context->currentFieldLength - context->currentFieldPos))
                     ? context->commandLength
                     : context->currentFieldLength - context->currentFieldPos);
            copyClauseData(context, context->content->data + context->currentFieldPos, copySize);
        }
        if (context->currentFieldPos == context->currentFieldLength) {
            context->currentField++;
            context->processingField = false;
    ...
    for (;;) {
        ...
        if (!context->processingField) {
            ...
            while (context->commandLength != 0) {
                ...
                context->rlpBuffer[context->rlpBufferPos++] = readClauseByte(context);
