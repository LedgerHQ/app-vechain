...
bool maxFeeToDisplayString(txInt256_t *gaspricecoef,
    ...
    convertUint256BE(gaspricecoef->value,
                     gaspricecoef->length,
                     &feeComputationContext->gasPriceCoef);
    convertUint256BE(gas->value, gas->length, &feeComputationContext->gas);
    // (BGP * GPC)
    mul256(&feeComputationContext->gasPriceCoef,
           &feeComputationContext->baseGasPrice,
           &feeComputationContext->tmp);

    // (BGP / 255) * GPC
    divmod256(&feeComputationContext->tmp,
              &feeComputationContext->maxGasCoef,
              &feeComputationContext->maxFeeBonus,
              &feeComputationContext->gasPriceCoef);
    // (1 + BGP / 255) * GPC
    add256(&feeComputationContext->maxFeeBonus,
           &feeComputationContext->baseGasPrice,
           &feeComputationContext->tmp);
    // (1 + BGP / 255) * GPC) * G
    mul256(&feeComputationContext->tmp,
